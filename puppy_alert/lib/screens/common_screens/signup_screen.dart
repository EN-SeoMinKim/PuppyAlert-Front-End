  ...
  void _searchAddress(BuildContext context) async {
    ...
    void _clickDuplicationButton(bool isId) async {
      ...
      if (isId) {
        ...
        uri =
            Uri.parse(
                '${dotenv.get('BASE_URL')}/common/checkId?id=$inputString');
      } else {
        key = _nickNameFormKey;
        String inputString = _textEditingController[widget._nickName].text
            .trim();
        uri = Uri.parse(
            '${dotenv.get(
                'BASE_URL')}/common/checkNickName?nickName=$inputString');
      }
      ...
      // 중복이 없는 입력이라면
      if (!jsonDecode((await http.get(uri)).body)['isExists']) {} else {
        // 중복이 있다면
      }
    }
    ...
    Widget build(BuildContext context) {
      ...
      final Object arguments = ModalRoute
          .of(context)!
          .settings
          .arguments!;
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              SizedBox(
                                width: 200,
                                child: TextFormField(
                                  controller: _textEditingController[widget._postcode],
